<template>
  <NuxtLayout name="admin">
    <div class="space-y-4 font-sans pb-12">
      <!-- Header -->
      <div
        class="bg-white dark:bg-gray-950 border border-slate-200 dark:border-gray-800 p-3 shadow-xs flex flex-wrap items-center justify-between gap-3">
        <div class="flex items-center gap-3">
          <div
            class="w-9 h-9 bg-emerald-100 dark:bg-emerald-950/80 text-emerald-700 dark:text-emerald-400 flex items-center justify-center text-lg border border-emerald-300 dark:border-emerald-800/80">
            🛒
          </div>
          <div>
        ...
        <button @click="openCreateModal"
          class="px-4 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white font-medium text-xs shadow-sm flex items-center gap-1.5 transition-all">
          <span>+</span> Create PO
        </button>
      </div>
      ...
      <div class="bg-white dark:bg-gray-950 border border-slate-200 dark:border-gray-800 shadow-xs">
        <div class="overflow-x-auto">
          <table class="w-full text-left text-xs">
            <thead
              class="bg-slate-100 dark:bg-gray-900 border-b border-slate-200 dark:border-gray-800 text-[11px] text-slate-600 dark:text-gray-400 uppercase tracking-wide">
              <tr>
            ...
            <tbody class="divide-y divide-slate-200 dark:divide-gray-800 font-mono">
              ...
              <tr v-else v-for="order in paginatedData" :key="order.id"
                class="hover:bg-slate-50 dark:hover:bg-gray-900/50 transition-colors">
                ...
                <td class="py-2.5 px-3 text-center font-sans">
                  <button @click="viewOrderDetails(order)"
                    class="text-xs bg-slate-200 dark:bg-gray-800 hover:bg-slate-300 dark:hover:bg-gray-700 text-slate-700 dark:text-gray-200 px-2 py-1 transition-colors">
                    View
                  </button>
                </td>
              </tr>
        ...
        <!-- Pagination Footer -->
        <PaginationControls :current-page="currentPage" :total-pages="totalPages" :total-items="orders.length"
      ...
      <div v-if="showAddModal"
        ...
        <div
          class="bg-white dark:bg-gray-950 border border-slate-200 dark:border-gray-800 w-full max-w-2xl shadow-xl flex flex-col max-h-[90vh]">
          <div
            class="p-3 border-b border-slate-200 dark:border-gray-800 flex items-center justify-between bg-slate-50 dark:bg-gray-900">
            <h3 class="font-semibold text-slate-800 dark:text-gray-100 text-sm">Create Purchase Order</h3>
            <button @click="showAddModal = false"
              class="text-slate-400 hover:text-slate-600 dark:hover:text-gray-200 cursor-pointer text-lg leading-none">×</button>
          </div>

          <div class="p-4 overflow-y-auto flex-1 text-xs space-y-4">
            <div class="grid grid-cols-2 gap-4">
              <div>
                <label class="block font-medium text-slate-700 dark:text-gray-300 mb-1">Select Supplier <span
                    class="text-red-500">*</span></label>
                <select v-model="form.supplierId" required
                  class="w-full bg-white dark:bg-gray-900 border border-slate-300 dark:border-gray-700 px-2.5 py-1.5 text-slate-800 dark:text-gray-100 outline-none focus:border-emerald-500">
            ...
            <div class="border border-slate-200 dark:border-gray-800 bg-slate-50 dark:bg-gray-900 p-3">
              <h4 class="font-semibold text-slate-700 dark:text-gray-300 mb-2 flex items-center justify-between">
                ...
                <button type="button" @click="addItemRow"
                  class="text-emerald-600 dark:text-emerald-400 hover:underline">+ Add Medicine</button>
              </h4>

              <div class="space-y-2">
                <!-- Header for Order Items -->
                <div v-if="form.items.length > 0"
                  class="flex gap-2 items-center px-1 pb-1 border-b border-slate-200 dark:border-gray-700 text-xs font-semibold text-slate-600 dark:text-gray-400">
                ...
                <div v-for="(item, idx) in form.items" :key="idx" class="flex gap-2 items-center">
                  <select v-model="item.productId"
                    class="flex-1 bg-white dark:bg-gray-950 border border-slate-300 dark:border-gray-700 px-2 py-1.5 outline-none focus:border-emerald-500"
                    @change="autoFillCost(item)">
                    <option value="" disabled>Select Medicine...</option>
                    <option v-for="prod in products" :key="prod.id" :value="prod.id">
                      {{ prod.name }} ({{ prod.generic_name || 'Generic' }})
                    </option>
                  </select>
                  <input v-model.number="item.quantity" type="number" min="1" placeholder="Qty" title="Quantity"
                    class="w-20 bg-white dark:bg-gray-950 border border-slate-300 dark:border-gray-700 px-2 py-1.5 outline-none font-mono text-center" />
                  <input v-model.number="item.unitCost" type="number" step="0.01" min="0" placeholder="Unit Cost"
                    title="Unit Cost"
                    class="w-28 bg-white dark:bg-gray-950 border border-slate-300 dark:border-gray-700 px-2 py-1.5 outline-none font-mono text-right" />
                  ...
                  <button type="button" @click="removeItemRow(idx)"
                    class="text-red-500 hover:text-red-700 px-1 w-5 text-center" title="Remove Item">✕</button>
                </div>
              </div>

              <div
                class="mt-4 pt-2 border-t border-slate-200 dark:border-gray-800 flex justify-between items-center font-semibold text-sm">
                <span class="text-slate-700 dark:text-gray-300 uppercase tracking-wider text-[11px]">Total Estimated
                  Cost:</span>
          ...
          <div
            class="p-3 border-t border-slate-200 dark:border-gray-800 bg-slate-50 dark:bg-gray-900 flex justify-end gap-2 text-xs">
            <button type="button" @click="showAddModal = false"
              class="px-3 py-1.5 bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 dark:bg-gray-800 dark:border-gray-700 dark:text-gray-300">Cancel</button>
            <button type="button" @click="submitOrder"
              :disabled="submitting || !form.supplierId || form.items.length === 0"
              class="px-4 py-1.5 bg-emerald-600 text-white font-medium hover:bg-emerald-700 disabled:opacity-50">
      ...
      <!-- ========================================== -->
      <div v-if="viewModalOpen && selectedPO"
        class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4 z-50">
        <div
          class="bg-white dark:bg-gray-950 border border-slate-200 dark:border-gray-800 w-full max-w-3xl shadow-xl flex flex-col max-h-[90vh]">
          <div
            class="p-4 border-b border-slate-200 dark:border-gray-800 flex items-center justify-between bg-slate-50 dark:bg-gray-900">
            <div>
              ...
              <p class="text-[11px] text-slate-500 dark:text-gray-400 mt-0.5">Supplier: {{ selectedPO.supplierName }}
              </p>
            </div>
          ...
          <div class="p-4 overflow-y-auto flex-1 text-xs">
            ...
            <div v-else>
              <table class="w-full text-left font-sans">
                <thead
                  class="bg-slate-100 dark:bg-gray-900 border-b border-slate-200 dark:border-gray-800 text-[10px] text-slate-600 dark:text-gray-400 uppercase">
                  <tr>
                ...
                <tbody class="divide-y divide-slate-200 dark:divide-gray-800 font-mono">
                  <tr v-for="item in poDetails?.items" :key="item.id">
                    ...
                    <td class="py-2 px-2 text-right text-slate-600 dark:text-gray-400">{{ settingsStore.currencySymbol
                      }}{{ item.unitCost.toFixed(2) }}</td>
                    <td class="py-2 px-2 text-right font-medium text-slate-800 dark:text-gray-200">{{
                      settingsStore.currencySymbol }}{{ item.lineTotal.toFixed(2) }}</td>
                  </tr>
                </tbody>
                <tfoot>
                  <tr class="bg-slate-50 dark:bg-gray-900/50">
                    <td colspan="3"
                      class="py-2 px-2 text-right font-sans font-semibold text-slate-700 dark:text-gray-300 text-[11px] uppercase tracking-wider">
                      Total Amount:</td>
                    <td class="py-2 px-2 text-right font-mono font-bold text-emerald-700 dark:text-emerald-400">{{
                      settingsStore.currencySymbol }}{{ poDetails?.totalAmount.toFixed(2) }}</td>
                  </tr>
                </tfoot>
              </table>

              <!-- Receive Form Section -->
              <div v-if="receiveMode"
                class="mt-6 border border-emerald-200 dark:border-emerald-900 bg-emerald-50 dark:bg-emerald-950/20 p-3">
                <h4 class="font-semibold text-emerald-800 dark:text-emerald-400 mb-2 flex items-center gap-1.5">
                  <span>📦</span> Stock Receipt Form
                </h4>
                <p class="text-[10px] text-emerald-600 dark:text-emerald-500 mb-3">Please specify the received Batch
                  Numbers and Expiry Dates before moving items into inventory stock.</p>

                <div class="space-y-3 font-sans">
                  <div v-for="(item, idx) in poDetails?.items" :key="'recv-' + item.id"
                    class="flex flex-col sm:flex-row gap-2 sm:items-center bg-white dark:bg-gray-900 p-2 border border-emerald-100 dark:border-emerald-900">
                    ...
                    <div class="flex gap-2">
                      <input v-model="receiveForm[idx].batchNumber" type="text" placeholder="Batch No (Opt)"
                        class="w-32 bg-slate-50 dark:bg-gray-950 border border-slate-200 dark:border-gray-800 px-2 py-1 outline-none text-[11px] font-mono" />
                      <input v-model="receiveForm[idx].expiryDate" type="date"
                        class="w-32 bg-slate-50 dark:bg-gray-950 border border-slate-200 dark:border-gray-800 px-2 py-1 outline-none text-[11px] font-mono" />
                    </div>
                  </div>
                </div>
          ...
          <!-- Modals Actions -->
          <div
            class="p-3 border-t border-slate-200 dark:border-gray-800 bg-slate-50 dark:bg-gray-900 flex justify-between items-center text-xs">
            <!-- Left Actions -->
            <div class="flex gap-2">
              <button v-if="selectedPO.status === 'DRAFT'" @click="updateStatus('SENT')" :disabled="poActionLoading"
                class="px-3 py-1.5 bg-blue-600 text-white font-medium hover:bg-blue-700 disabled:opacity-50">
                Mark as SENT
              </button>
              <button v-if="selectedPO.status === 'DRAFT' || selectedPO.status === 'SENT'"
                @click="updateStatus('CANCELLED')" :disabled="poActionLoading"
            ...
            <!-- Right Actions -->
            <div class="flex gap-2">
              <button v-if="receiveMode" @click="receiveMode = false"
                class="px-3 py-1.5 bg-white border border-slate-300 text-slate-700 hover:bg-slate-50 dark:bg-gray-800 dark:text-gray-300 dark:border-gray-700">Back</button>

              <button v-if="(selectedPO.status === 'DRAFT' || selectedPO.status === 'SENT') && !receiveMode"
                @click="enterReceiveMode" :disabled="poActionLoading"
              ...
              <button v-if="receiveMode" @click="submitReceive" :disabled="poActionLoading"
