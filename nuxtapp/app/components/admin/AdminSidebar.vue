  ...
  <div>
    ...
    <aside :class="[
      ...
      <div class="flex-1 flex flex-col overflow-hidden">
        <!-- Sidebar Tree Header -->
        <div
          class="px-2.5 py-1.5 bg-[#e9eef2] dark:bg-gray-900 border-b border-slate-300 dark:border-gray-800 text-[11px] font-normal text-slate-700 dark:text-gray-300 flex items-center justify-between">
          <span class="flex items-center gap-1">
            <span class="text-xs">📂</span> Explorer Navigation
          </span>
          <span
            class="font-mono text-[9px] bg-slate-300 dark:bg-gray-800 px-1 py-0.2 text-slate-700 dark:text-gray-300 font-normal">TREE</span>
        </div>
        ...
        <nav class="p-2 space-y-1.5 overflow-y-auto flex-1 font-sans text-xs">
          <NuxtLink v-for="link in navLinks" :key="link.path" :to="link.path" @click="closeSidebar"
            ...
            <span :class="route.path === link.path ? 'text-white' : 'text-slate-500 dark:text-gray-400'"
      ...
      <div class="p-2 border-t border-slate-300 dark:border-gray-800 bg-[#eef2f5] dark:bg-gray-900 text-xs font-sans">
        <NuxtLink to="/pos" @click="closeSidebar"
...
const { isBranchManager, isCashier, isStoreAdmin, isSuperAdmin } = useAuth();
...
const allNavLinks = [
  ...
  { label: 'Store Branches', path: '/admin/branches', icon: '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"></path></svg>', ownerOnly: true },
  ...
  { label: 'Billing & Plan', path: '/admin/billing', icon: '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 10h18M7 15h1m4 0h1m-7 4h12a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z"></path></svg>', ownerOnly: true },
  { label: 'Store Settings', path: '/admin/settings', icon: '<svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0-3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path></svg>', ownerOnly: true }
];

const navLinks = computed(() => {
  if (isBranchManager.value || isCashier.value) {
    return allNavLinks.filter(link => !link.ownerOnly);
  }
  return allNavLinks;
});
</script>
