<template>
  <div
    class="bg-[#f0f3f6] dark:bg-gray-950 border-b border-slate-300 dark:border-gray-800 px-2.5 py-1.5 flex flex-wrap md:flex-nowrap items-center justify-between gap-2 select-none text-xs font-sans">
    <!-- Left: Category Pills List -->
    <div class="flex items-center gap-1 shrink-0 overflow-x-auto scrollbar-none max-w-full">
      <span
        class="font-normal text-[11px] text-slate-600 dark:text-gray-400 uppercase tracking-wider shrink-0 mr-1 flex items-center gap-1">
        <span>📁</span> CATEGORY:
      </span>

      <button v-for="cat in categories" :key="cat.id" @click="activeCategory = cat.name" :class="[
    ...
    <div class="shrink-0 flex items-center gap-2 order-2 md:order-3">
      ...
      <div v-if="tenantPlan"
        class="flex items-center gap-1 px-2 py-0.5 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-500/30 text-emerald-700 dark:text-emerald-400 font-mono font-normal text-[11px]">
        <span>🛡️</span>
