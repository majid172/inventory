...
const products = ref<POSProduct[]>(defaultProducts);
const activeCategory = ref<string>("All Items");
const searchQuery = ref<string>("");
const loading = ref<boolean>(false);

const categoriesList = ref<string[]>(["All Items", "Hot Drinks", "Cold Drinks", "Bakery", "Retail Coffee", "Merchandise"]);

const categories = computed<POSCategory[]>(() => {
  return categoriesList.value.map((catName, index) => {
    const count = catName === "All Items" 
      ? products.value.length 
      : products.value.filter(p => p.category && p.category.toLowerCase() === catName.toLowerCase()).length;
...
const filteredProducts = computed(() => {
  ...
  const cat = (activeCategory.value || 'All Items').toLowerCase();
...
export function usePOSProducts() {
  const fetchProducts = async () => {
    ...
    try {
      ...
      console.warn('Backend API offline, using local catalog.');
    } finally {
      loading.value = false;
    }
  };
